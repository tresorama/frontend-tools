<script lang="ts">
	...
	$: outpuCode = convertInputToOutput(inputCode);

	let status: 'error' | 'success';
	$: status = outpuCode.startsWith('Invalid input code') ? 'error' : 'success';

	let outpuCodePhp: string | null = null;
	$: if (browser && status === 'success') convertOutputToOutputPhp();
	$: if (browser && status === 'error') outpuCodePhp = null;

	function convertInputToOutput(sourceValue: string) {
		try {
	...
	// events
	async function convertOutputToOutputPhp() {
		// fetcher
		const transformJsonToPhpArray = async (inputCode: string): Promise<{ outputCode: string }> => {
			const url = window.location.pathname + '/api/transform-js-to-php';
		...
		// call
		try {
			const result = await transformJsonToPhpArray(outpuCode);
			outpuCodePhp = result.outputCode;
			toast.success('PHP code generated');
		} catch (error) {
			console.error(error);
			outpuCodePhp = 'Error: ' + (error instanceof Error ? error.message : 'Unknown error');
			toast.error('Error while generating PHP code');
		}
	}
...
<div class="layout">
	...
	<section>
		<h2>Output</h2>
		<h3>JSON code</h3>
		<button type="button" on:click={() => handleCopyToClipBoard(outpuCode)}>
			Copy to Clipboard
		</button>
		<p>
		...
		<h3>PHP code (beta)</h3>
		<button type="button" on:click={() => handleCopyToClipBoard(outpuCodePhp ?? '')}>
			Copy to Clipboard
		</button>

		<div class="editor-wrapper output-php">
			<Monaco
				options={{
					language: 'php',
					automaticLayout: true,
